...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // CONFIGURAÇÕES VIA ENV OU FIXAS
  ...
  const versao = "07"; // CONTROLE DE VERSÃO MANUAL
  ...
  try {
    for (const ativo of ATIVOS) {
      // Monitoramento em M15 como base para o Telegram
      ...
      const isVelaVerde = c[i].c > c[i].o;
      const isVelaVermelha = c[i].c < c[i].o;

      let sinalStr = "";

      // LÓGICA SINAL ACIMA
      if (e4_prev <= e8_prev && e4_atual > e8_atual && rsi9 > 50 && rsi9 > rsi9_prev && isVelaVerde) {
        sinalStr = "ACIMA";
      }
      // LÓGICA SINAL ABAIXO
      if (e4_prev >= e8_prev && e4_atual < e8_atual && rsi9 < 50 && rsi9 < rsi9_prev && isVelaVermelha) {
        sinalStr = "ABAIXO";
      }
    ...
    return res.status(200).send(`
      ...
      <body>
          <div class="main-card">
              ...
              <div class="history">
                Versão 00: Elaboração Inicial<br>
                Versão 01 a 05: Ajustes de Fractais e MACD<br>
                Versão 06: Remoção de Fractais e Implementação EMA 9/21<br>
                Versão 07: Lógica EMA 4/8 + RSI 9 + Cor da Vela (Atual)
              </div>
          </div>
