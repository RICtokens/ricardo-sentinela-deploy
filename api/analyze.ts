...
// Trava global para evitar repetição no mesmo minuto/ativo
let lastSinais: Record<string, string> = {};

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const token = "8223429851:AAFl_QtX_Ot9KOiuw1VUEEDBC_32VKLdRkA";
  const chat_id = "7625668696";
  const versao = "00-M1-PRO-TRAVADO";
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      // --- CÁLCULOS INDICADOR RICARDO TRADER ---
      ...
      if (sinalStr) {
        // ID ÚNICO: ATIVO + HORA DA VELA (Ex: USDJPY_ABAIXO_1738799520)
        const sid = `${ativo.label}_${sinalStr}_${c[i].t}`;
        
        // SÓ ENVIA SE O ID FOR DIFERENTE DO ÚLTIMO ENVIADO
