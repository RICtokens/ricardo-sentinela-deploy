...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Regra de Janela de Compra M15 (Melhoria 3)
  // O robô só envia sinal se estiver nos primeiros 10 minutos da vela (ex: 09:00 até 09:10)
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      // LÓGICA DE SINAL (Melhoria 1: Cores Invertidas p/ Bater com Sentido do Trade)
      if (e4_p <= e8_p && e4_i > e8_i && (rsi_i > 30 || rsi_i > 50) && rsi_i > rsi_p && isVerde) {
        sinalStr = "ACIMA";
        iconSinal = "🟢"; // Verde para ACIMA
        setaSinal = "⬆";
      } else if (e4_p >= e8_p && e4_i < e8_i && (rsi_i < 70 || rsi_i < 50) && rsi_i < rsi_p && isVermelha) {
        sinalStr = "ABAIXO";
        iconSinal = "🔴"; // Vermelha para ABAIXO
        setaSinal = "⬇";
      }

      // Envio de sinal com trava de tempo (Melhoria 2 e 3)
      if (sinalStr && dentroDaJanela) {
        ...
        if (lastSinais[ativo.label] !== signalKey) {
          ...
          const msg = `${iconSinal} <b>SINAL EMITIDO!</b>\n<b>ATIVO</b>: ${ativo.label}\n<b>SINAL</b>: ${setaSinal} ${sinalStr}\n<b>VELA</b>: ${hVela}`;
