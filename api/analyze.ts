...
// Memória temporária (Atenção: em Serverless, isso é volátil)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // --- REGRAS DE ABERTURA CONFORME OPTNEX ---
  ...
  const ATIVOS = [
    ...
    // Alterado endpoint Yahoo para maior estabilidade na v64
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      try {
        ...
        // Se falhar o carregamento, registra erro interno para log
        ...
        if (sinalStr) {
          ...
          // Chave de unicidade inclui Ativo + Hora da Vela para evitar duplicidade no Telegram
          ...
          if (!lastSinais[opId]) {
            ...
            // Formato de mensagem aprovado (sem espaçamento extra)
