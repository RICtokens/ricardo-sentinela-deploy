...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // --- CONFIGURAÇÃO DE TEMPO PRECISA ---
  const agora = new Date();
  const timeZone = 'America/Sao_Paulo';
  
  // Obter hora e minuto exatos de SP
  const horaSP = parseInt(agora.toLocaleTimeString('pt-BR', { timeZone, hour: '2-digit', hour12: false }));
  const minSP = parseInt(agora.toLocaleTimeString('pt-BR', { timeZone, minute: '2-digit', hour12: false }));
  const horaFormatada = (horaSP * 100) + minSP;

  // Obter dia da semana de SP (0=Dom, 1=Seg... 6=Sab)
  const diaSemana = parseInt(new Intl.DateTimeFormat('en-US', { timeZone, weekday: 'numeric' }).format(agora)) % 7;

  const dataExibicao = agora.toLocaleDateString('pt-BR', { timeZone });
  const horaExibicao = agora.toLocaleTimeString('pt-BR', { timeZone, hour: '2-digit', minute: '2-digit' });
