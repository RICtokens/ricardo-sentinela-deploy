...
// Memória temporária para evitar duplicatas (Persiste enquanto a instância Vercel estiver ativa)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const getStatus = (label: string): boolean => {
    // SE VOCÊ QUISER TESTAR EURUSD AGORA (MERCADO FECHADO), MUDE PARA "return true" ABAIXO:
    if (label === "BTCUSD") return true;
    if (label === "EURUSD") return true;
      if (diaSemana >= 1 && diaSemana <= 4) return (horaFormatada >= 0 && horaFormatada <= 1800) || (horaFormatada >= 2200 && horaFormatada <= 2359);
    ...
    for (const ativo of ATIVOS) {
      ...
      if (ativo.source === "kucoin") {
        candles = json.data.map((v: any) => {
          ...
          return { t, o: parseFloat(v[1]), c: parseFloat(v[2]), h: parseFloat(v[3]), l: parseFloat(v[4]) };
        }).reverse();
      } else {
      ...
      // --- LÓGICA ESPELHO RT_ROBO_V.01 ---
      // Fractal i-2 comparado com os 2 anteriores (i-4, i-3) e 2 posteriores (i-1, i)
      ...
      if (f_alta && rsi_call_ok && candles[i].c > candles[i].o) {
        sinalStr = "ACIMA"; seta = "↑"; emoji = "🟢";
      }
      if (f_baixa && rsi_put_ok && candles[i].c < candles[i].o) {
        sinalStr = "ABAIXO"; seta = "↓"; emoji = "🔴";
      }

      if (sinalStr) {
        // ID de unicidade: Versão + Ativo + Timestamp da vela central do fractal
        const opId = `v${versao}_${ativo.label}_${candles[i-2].t}`;
        
        if (!lastSinais[opId]) {
          lastSinais[opId] = true;
          const hVela = new Date(candles[i].t).toLocaleTimeString('pt-BR', { timeZone, hour: '2-digit', minute: '2-digit' });
          
          // FORMATO TELEGRAM APROVADO (SEM ESPAÇAMENTO EXTRA)
