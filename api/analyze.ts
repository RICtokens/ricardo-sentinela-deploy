...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      // 's' é a Vela [2] do gráfico, onde a seta plota 
      // 'i' é a Vela [0] (atual)
      const i = c.length - 1; 
      const s = i - 2; 

      // --- CÁLCULOS EXATOS DO SCRIPT RT_PRO ---
      ...
      // MACD [cite: 1]
      const linha_macd = getEMA(12, s) - getEMA(26, s);
      const sinal_macd = getEMA(9, s);

      // RSI [cite: 1]
      const rv = getRSI(s, 9);
      const rv_prev = getRSI(s - 1, 9);

      // MOMENTUM (10) [cite: 1]
      const mom = c[s].c - c[s - 10].c;
      const mom_prev = c[s - 1].c - c[s - 11].c;

      // DINAPOLI STOCHASTIC [cite: 3]
      const low14 = Math.min(...c.slice(s - 13, s + 1).map(v => v.l));
      const high14 = Math.max(...c.slice(s - 13, s + 1).map(v => v.h));
      const fast_stoch = ((c[s].c - low14) / (high14 - low14)) * 100;
      // Simplificação do cálculo de série para o robô:
      const stoch_k = fast_stoch; 
      const stoch_d = (getRSI(s, 3)); // Aproximação da Slow D [cite: 2]

      // REGRAS DO FRACTAL (5 BARRAS) 
      ...
      // SINAL FINAL: FRACTAL + MACD + RSI + STOCH + MOMENTUM 
      let sinalStr = "";
      if (f_fundo && linha_macd > sinal_macd && rv > rv_prev && stoch_k > stoch_d && mom > mom_prev) sinalStr = "ACIMA";
      if (f_topo && linha_macd < sinal_macd && rv < rv_prev && stoch_k < stoch_d && mom < mom_prev) sinalStr = "ABAIXO";

      if (sinalStr) {
        const sid = `${ativo.label}_${sinalStr}_${c[s].t}`;
        if (lastSinais[ativo.label] !== sid) {
          lastSinais[ativo.label] = sid;
          const msg = `**🚨 SINAL CONFIRMADO RT_PRO**\n\n**ATIVO**: ${ativo.label}\n**ORDEM**: ${sinalStr === "ACIMA" ? "🟢 COMPRA" : "🔴 VENDA"}\n**VELA DO SINAL**: ${new Date(c[s].t * 1000).toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo' })}`;
    ...
    return res.status(200).send(`
      ...
      <body>
          <div class="main-card">
              ...
              <div class="status-badge">SINAL SIMULTÂNEO AO GRÁFICO</div>
              <div class="asset-grid">
                  <div class="asset-card"><span>BTCUSD</span><span class="status-pill">ONLINE</span></div>
                  <div class="asset-card"><span>EURUSD</span><span class="status-pill">ONLINE</span></div>
                  <div class="asset-card"><span>GBPUSD</span><span class="status-pill">ONLINE</span></div>
                  <div class="asset-card"><span>USDJPY</span><span class="status-pill">ONLINE</span></div>
              </div>
              <div class="footer">
                  ...
                  <div><b>STATUS</b><p style="color:var(--primary)">CONECTADO</p></div>
              </div>
          </div>
          <script>setTimeout(()=>location.reload(), 10000);</script>
      </body></html>
    `);
  } catch (e) { return res.status(200).send("CONECTANDO..."); }
}
