...
// Memória volátil - Trava de duplicidade
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      try {
        ...
        // --- NORMALIZAÇÃO DE TIMESTAMPS (S para MS) ---
        ...
        // LÓGICA FRACTAL i-2 (ALINHAMENTO OPTNEX)
        ...
        let sinalStr = "";
        if (f_alta && rsi_val > rsi_ant && rsi_val >= 30 && candles[i].c > candles[i].o) sinalStr = "ACIMA";
        if (f_baixa && rsi_val < rsi_ant && rsi_val <= 70 && candles[i].c < candles[i].o) sinalStr = "ABAIXO";

        if (sinalStr) {
          // ID DE UNICIDADE REFORÇADO: Ativo + Tempo da Vela + Direção
