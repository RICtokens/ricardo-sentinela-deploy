...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // --- REGRAS DE ABERTURA CONFORME ITEM 5 (PRECISÃO TOTAL) ---
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      try {
        ...
        // Validação mínima para Fractal i-2 (mínimo 5 candles)
        if (candles.length < 10) continue;
        
        // i = Vela Atual (em formação), i-1 = Vela Anterior Fechada
        ...
        // O fractal no índice i-2 é confirmado pelas velas i-4, i-3 (passado) e i-1, i (futuro/atual)
        ...
        let sinalStr = "";
        // Validação da cor da vela atual (candles[i]) + Fractal + RSI
        ...
        if (sinalStr) {
          ...
          // ID único para evitar repetição na mesma vela do mesmo ativo
          ...
          if (!lastSinais[opId]) {
            ...
            // FORMATO TELEGRAM APROVADO (SEM ESPAÇOS EXTRAS)
