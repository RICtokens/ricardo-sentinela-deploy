...
// Memória para evitar duplicidade de sinais
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // --- REGRAS DE ABERTURA EURUSD CONFORME OPTNEX ---
  const getStatus = (label: string): boolean => {
    ...
    if (label === "EURUSD") {
      ...
      if (diaSemana === 5) return (horaFormatada >= 0 && horaFormatada <= 1630); // Sex
      if (diaSemana === 0) return (horaFormatada >= 2200 && horaFormatada <= 2359); // Dom
    }
    return false;
  };
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      const resApi = await fetch(urlM15, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
      ...
      // LÓGICA FRACTAL OPTNEX (Baseada no script RT_ROBO_V.01)
      ...
      if (sinalStr) {
        ...
        if (!lastSinais[opId]) {
          ...
          // Formatação Telegram Aprovada (Sem espaçamento extra e negritos corretos)
