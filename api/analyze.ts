...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const versao = "77"; // Atualizada conforme Briefing
  ...
  // Horários Oficiais Optnex EURUSD 
  const getStatus = (label: string): boolean => {
    ...
    if (label === "EURUSD") {
      ...
      return !(horaMinutoInt >= 1801 && horaMinutoInt <= 2159); // Seg-Qui fecha 18:01-21:59
    }
    return false;
  };
  ...
  // Melhoria: Função para Calcular EMA 20 
    ...
    for (const ativo of ATIVOS) {
      ...
      try {
        let url = ativo.source === "kucoin" 
          ...
          : `https://api.binance.com/api/v3/klines?symbol=${ativo.symbol}&interval=15m&limit=100`;
        ...
        for (let j = 0; j < 3; j++) {
          ...
          let sinalStr = "";
          // Lógica ACIMA (CALL) V.02 
          if (f_alta && (rsi_val >= 55 || rsi_val >= 30) && rsi_subindo && candles[i].c > ema_20 && vela_verde) {
            sinalStr = "ACIMA";
          }
          // Lógica ABAIXO (PUT) V.02 
          if (f_baixa && (rsi_val <= 45 || rsi_val <= 70) && rsi_caindo && candles[i].c < ema_20 && vela_vermelha) {
            sinalStr = "ABAIXO";
          }
          ...
          if (sinalStr) {
            ...
            if (!lastSinais[opId]) {
              ...
              // Formato Telegram Preservado (Regra de Ouro) 
